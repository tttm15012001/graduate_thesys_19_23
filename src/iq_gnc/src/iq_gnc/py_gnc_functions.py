...
from geometry_msgs.msg import Pose, PoseStamped, Point, Quaternion, PointStamped, TransformStamped
...
class gnc_api:
    def __init__(self):
        ...
        self.needCheckReach = True

        self.frameData = TransformStamped()
        self.masterFrame = [0, 0, 0]
        ...
        self.global_position = rospy.Subscriber(name='{}mavros/global_position/global'.format(self.ns), data_class=NavSatFix, callback=self.global_pos_callback)
    ...
    def set_destination(self, x, y, z, psi):
        ...
        # print('local: ' + str(Xlocal) + ' ' + str(Ylocal) + ' ' + str(Zlocal))
        ...
        # rospy.loginfo(
        #     "Destination set to x:{} origin frame".format(str(self.local_offset_pose_g.x)))

        self.waypoint_g.pose.position = Point(x, y, z)
        self.frameData.transform.translation.x = self.waypoint_g.pose.position.x
        self.frameData.transform.translation.y = self.waypoint_g.pose.position.y
        self.frameData.transform.translation.z = self.waypoint_g.pose.position.z
    ...
    def takeoff(self, takeoff_alt):
        ...
        """
        self.arm()
        self.set_destination(0, 0, takeoff_alt, 0)
    ...
    def check_waypoint_reached(self, pos_tol=0.3, head_tol=0.01):
        ...
        # rospy.loginfo(CBLUE2 + str(self.waypoint_g.pose.position.x) + " " + str(self.waypoint_g.pose.position.y) + " " + str(self.waypoint_g.pose.position.z) + CEND)
        # rospy.loginfo(CGREEN2 + str(self.current_pose_g.pose.pose.position.x) + " " + str(self.current_pose_g.pose.pose.position.y) + " " + str(self.current_pose_g.pose.pose.position.z) + CEND)
        ...
        # rospy.loginfo(CBLUE2 + str(dx) + " " + str(dy) + " " + str(dz) + CEND)

        dMag = sqrt(pow(dx, 2) + pow(dy, 2) + pow(dz, 2))
        # rospy.loginfo(CGREEN2 + str(dMag) + CEND)
        ...
        # rospy.loginfo(CBLUE2 + str(cosErr) + " cc " + str(sinErr) + CEND)

        dHead = sqrt(pow(cosErr, 2) + pow(sinErr, 2))
        # rospy.loginfo(CGREEN2 + str(dHead) + CEND)
    ...
    def calculateCoordinates(self, coordinates):
        coordinates[0] = int(coordinates[0]) - self.current_pose_g.pose.pose.position.x
        coordinates[1] = int(coordinates[1]) - self.current_pose_g.pose.pose.position.y
        # rospy.loginfo(CBLUE2 + str(self.current_pose_g.pose.pose.position.x) + self.ns + str(self.current_pose_g.pose.pose.position.y) + CEND)
        # rospy.loginfo(CGREEN2 + str(coordinates[0]) + self.ns + str(coordinates[1]) + CEND)
        # coordinates[2] = int(coordinates[2]) - self.current_pose_g.pose.pose.position.z
        # if(coordinates[0] < 0):
        #     coordinates[0] = 0 - coordinates[0]
        # if(coordinates[1] < 0):
        #     coordinates[1] = 0 - coordinates[1]
        # if(coordinates[2] < 0):
        #     coordinates[2] = 0 - coordinates[2]

    def set_destination_sub_drones(self, x, y, z, psi, topicChild, topicParent):
        """This function is used to command the drone to fly to a waypoint. These waypoints should be specified in the local reference frame. This is typically defined from the location the drone is launched. Psi is counter clockwise rotation following the drone's reference frame defined by the x axis through the right side of the drone with the y axis through the front of the drone.
        ...
        # # self.waypoint_g.pose.position = Point(x, y, z)
        buffer = Buffer()
        distanceFromServerFrame = self.frameData
        distanceMasterFrameFromServer = self.masterFrame

        # buffer = Buffer()
        self.waypoint_g.pose.position.x = distanceMasterFrameFromServer[0] + x
        self.waypoint_g.pose.position.y = distanceMasterFrameFromServer[1] + y
        self.waypoint_g.pose.position.z = distanceMasterFrameFromServer[2] + z
        temp = self.waypoint_g
        local_target_pose = do_transform_pose(self.waypoint_g, distanceFromServerFrame)
        local_target_pose.pose.orientation.x = 0
        local_target_pose.pose.orientation.y = 0
        local_target_pose.pose.orientation.z = 0
        local_target_pose.pose.orientation.w = 0
        temp.pose.position = Point(local_target_pose.pose.position.x, local_target_pose.pose.position.y, local_target_pose.pose.position.z)
        print(temp)
        if temp.pose.position.x < 0:
            temp.pose.position.x = 0 - temp.pose.position.x
            # psi = 90
        self.set_heading(psi)
        temp.pose.orientation = self.waypoint_g.pose.orientation
        print(temp)
        self.local_pos_pub.publish(temp)

        self.transform.header.stamp = rospy.Time.now()
        self.transform.header.frame_id = topicParent

        # Set the child frame ID
        self.transform.child_frame_id = topicChild

        # Set the transform from the parent frame to the child frame
        self.transform.transform.translation.x = distanceFromServerFrame.transform.translation.x - local_target_pose.pose.position.x
