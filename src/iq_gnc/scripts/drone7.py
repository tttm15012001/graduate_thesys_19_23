...
def callback(data):
    ...
    if canReceive == True or data.data.find('drone7: ') != -1 or data.data.find('drone7true') != -1:
        rospy.loginfo(rospy.get_caller_id() + " I heard %s", data.data)
        if data.data == 'out' or data.data.find('drone7out') != -1:
            ...
            timer.start()
            manageDrone.updateState('drone7 ', PRE_LANDING)
        else:
            analizeData = data.data.split(': ')
            if analizeData[0] == TAKEOFF and manageDrone.state == LANDED:
                # if 'drone7true' in analizeData[1]:
                #     drone.takeoff(1)
                #     manageDrone.updateState('drone7 ', TAKING_OFF)
                drone.takeoff(1)
                manageDrone.updateState('drone7 ', TAKING_OFF)
            elif manageDrone.state == TAKEOFF and analizeData[0] == 'drone7':
                random_number = random.randrange(0, 11, 1) / 2.0
...
def main():
    ...
    rospy.init_node("drone7", anonymous=True)

    chatter_pub7 = rospy.Publisher(name='drone7_listen', data_class=String, queue_size=100)
    ...
    while not rospy.is_shutdown():
        ...
        if manageDrone.state == LANDING:
            ...
            if drone.check_waypoint_reached():
                manageDrone.updateState('drone7 ', LANDED)
        elif manageDrone.state == PRE_LANDING:
            if drone.check_waypoint_reached():
                manageDrone.updateState('drone7 ', LANDING)
                drone.land()
        if counter > 8 and drone.check_waypoint_reached():
            if drone.needCheckReach == True:
                data = 'reached:7'
                rospy.loginfo(data)
                chatter_pub7.publish(data)
                drone.needCheckReach = False
            global canReceive
            canReceive = True
        elif counter == 8:
            manageDrone.updateState('drone7 ', TAKEOFF)
            data = 'takeoff:7'
            rospy.loginfo(data)
            chatter_pub7.publish(data)
            counter += 1
