...
def callback(data):
    ...
    if canReceive == True or data.data.find('drone4: ') != -1 or data.data.find('drone4true') != -1:
        rospy.loginfo(rospy.get_caller_id() + " I heard %s", data.data)
        if data.data == 'out' or data.data.find('drone4out') != -1:
    ...
    while not rospy.is_shutdown():
        ...
        if manageDrone.state == LANDING:
            ...
            if drone.check_waypoint_reached():
                manageDrone.updateState('drone4 ', LANDED)
        elif manageDrone.state == PRE_LANDING:
            if drone.check_waypoint_reached():
                manageDrone.updateState('drone4 ', LANDING)
                drone.land()
